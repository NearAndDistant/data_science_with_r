...
flights_trans |>
  ...
  geom_line(aes(y = pos_pct, group = year, color = fct_rev(year)), linetype = "dashed", size = 0.5) +
  ...
  scale_colour_manual(NULL, values = rev(c("#e47176", "#329bce", "#346594")), guide_legend(reverse = TRUE)) +
...
eu_flights_trans |>
  ...
  geom_line(aes(y = 1), color = "black", linetype = "solid", alpha = 0.3, size = 0.5, show.legend = FALSE) +
  # years since pandemic
  geom_line(aes(y = pos_pct, group = year, color = fct_rev(year)), linetype = "dashed", size = 0.5, show.legend = FALSE) +
  # text for 2022 so far
  ...
  scale_y_continuous(breaks = c(0,0.5,1), labels = c("0%", "50%", "Pre-COVID Avg."), expand = c(0,0)) +
  coord_cartesian(clip = "off") +
  theme_minimal() +
  theme(legend.position    = c(0.05,0.1),
        ...
        panel.background   = element_rect(color = "#fff2e6", fill = "#fff2e6"),
        plot.background    = element_rect(color = "#fff2e6", fill = "#fff2e6"),
        plot.margin        = margin(0, 3, 1, 2.5, unit = "cm"))
...
ggdraw(xlim = c(0.02,0.98)) +
  ...
  draw_text("The Recovery of the Aviation Sector in Europe (Pre and Post Pandemic)", 
            x = 0.06, y = 0.97, hjust = 0, size = 20, family = "Montserrat SemiBold") + 
  draw_text("The number of flights as a proportion of the pre-pandemic average across months within each European country.", 
            x = 0.06, y = 0.945, hjust = 0, size = 12, family = "Montserrat") + 
