...
for(i in levels(dog_top_50$breed)) {
...
filter(breed == breed_i) %>%
  ...
  scale_x_continuous(limits = c(-0.5, max(dog_top_50$id)+1)) +
...
fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(width = 10, plotOutput("top_dogs"))
...
function(input, output) {
  ...
  renderPlot({
      ...
      draw_text("Each a Good Boy, Each a Character",    x = 0.5 , y = 0.90, size = 25, family = "amatic") +
      draw_text("Traits: Strong (5) to Weak (0)",       x = 0.5 , y = 0.86, size = 18, family = "amatic") +
      draw_text("Data: AKC | Graphic: @NearAndDistant", x = 0.42 , y = 0.35, size = 12, family = "me_again", hjust = 0, color = "grey70")
