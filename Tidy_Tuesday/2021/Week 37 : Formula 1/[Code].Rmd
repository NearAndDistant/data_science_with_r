...
f1_complete %>%
  ...
  mutate(constructor_lump = if_else(pc_point >= 0.10 , constructor , "Other")) %>%
  mutate(constructor_lump = factor(constructor , ordered = TRUE)) %>%
  ungroup() %>%
...
"Lotus-Climax"  = colorspace::lighten("#476b57", 0.25),
"Williams"      = colorspace::lighten("#005AFF", 0.25),
"Benetton"      = colorspace::lighten("#008860", 0.25),
"Other"         = "grey80"
)

levels = c("Ferrari" , "Maserati" , "Matra-Ford" , "Mercedes" , "Team Lotus" , "Red Bull"  , "McLaren" , "Lotus-Climax" , "Williams" ,"Benetton" ,  "Other")  
...
f1_tot_points %>%
  ggplot(aes(pc_point , season , fill = constructor_lump)) +
  geom_col(position = "stack" ,  show.legend = FALSE) +
  geom_point(data = f1_season_winners , x = 1.01 , aes(y = season , color = constructor) , show.legend = FALSE) +
  geom_text(data = f1_season_winners , x = 1.02  , aes(y = season , label = constructor_label) , hjust = 0 , family = text, show.legend = FALSE) +
  ...
  labs(title = "F1 Teams" , 
       ...
       x = "Season Points (%)" , y = NULL) +
  theme(text = element_text(family = text),
        plot.title = element_text(hjust = 0.05 , size = 30),
        plot.subtitle = element_text(hjust = 0.15 , size = 15),
        panel.grid = element_blank(),
        axis.text.x = element_text(size = 18),
        axis.title.x = element_text(size = 18 , vjust = -0.5),
        axis.text.y = element_text(size = 10 , margin = margin(r = -40)),
        plot.margin = margin(1,4,0.5,1, unit = "cm"))
